...
Finally, we will mark the `nationalparks` application as a backend for the map visualization tool, so that it can be dynamically discovered by the `parksmap-py` component using the OpenShift discovery mechanism and the map will be displayed automatically.
...
parksmap-py        1          1         1         config,image(parksmap-py:{{PARKSMAP_VERSION}})
----
...
[source]
----
http://nationalparks-userXXX-parks.apps.gshipley.pixy.io/ws/data/all
----
...
[source]
----
http://nationalparks-userXXX-parks.apps.gshipley.pixy.io/ws/data/load
----
...
Now that we have added the database and loaded the database, let's take a look at the parks map application.

[source]
----
http://parksmap-py-userXXX-parks.apps.gshipley.pixy.io
----
